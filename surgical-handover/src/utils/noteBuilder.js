...
export function buildUpdatePrompt(update) {
  ...
  const lines = [
    ...
    update.tempC ? `Temperature: ${update.tempC}°C` : '',
    update.bp ? `BP: ${update.bp} mmHg` : '',
    update.hr ? `HR: ${update.hr} bpm` : '',
    update.rr ? `RR: ${update.rr}/min` : '',
    update.spo2 ? `SpO2: ${update.spo2}` : '',
    update.wound ? `Wound: ${update.wound}` : '',
